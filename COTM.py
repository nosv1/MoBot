...
async def memberRemove(member, client):
  ...
  await channel.send(f"<@{member.id}> (a.k.a. {member.display_name}) is gone. {mo_member.mention}")
# end memberRemove
...
async def randomNiceMessage(client):
  phrases = [
    ...
    "Acknowledge your own, and others', ignorance. :)\nIt's less likely people are unable to learn and more likely they simply have not learned yet. Help them understand a point-of-view, rather than forcing them to change point-of-views.",
...
async def createOgVotingEmbed(message, member):
  ...
  extra_vote_hint = f"\nExtra spending votes can be received by posting a screenshot (or other form of proof) in this channel to prove you've completed the playlist (link in {message.guild.get_channel(VOTING).mention}).\n" if EXPERT_VOTER_ROLE not in [r.id for r in member.roles] else ""
...
def addRemovePitMarshal(host_pm, pit_marshals, member, member_divs, divs):
  ...
  if host_pm == 1: # if host
    ...
    avail = refineAvail(host_not_avail, member_divs)
    for div in hosts_needed:
      if div in avail and div in divs:
    ...
    avail = refineAvail(pm_not_avail, member_divs)
