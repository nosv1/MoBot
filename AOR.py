...
async def editDriverProfileOverviewEmbed(message, driverProfile):
  ...
  try:
    ...
    embed.set_author(name="AOR F1 Driver Profile: Overview", icon_url=moBotMember.avatar_url_as, url="https://google.com/user_id=%s/" % message.author.id)
...
async def openDriverProfileEmbed(message):
  ...
  moBotMember = message.guild.get_member(moBot)
  embed = discord.Embed(color=moBotMember.roles[-1].color)
  embed.set_author(name="AOR F1 Driver Profile: Select Name(s)", icon_url=moBotMember.avatar_url_as, url="https://google.com/user_id=%s/" % message.author.id)
  embed.set_thumbnail(url=message.guild.icon_url)
