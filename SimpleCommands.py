...
async def displayCommands(message, client):
  ...
  embed.set_author(name="MoBot Custom Commands", icon_url=client.user.avatar, url="https://google.com/SimpleCommands/")
...
async def deleteCommand(args, message, client):
  ...
  if (command is None):
    embed = discord.Embed(color=int("0xd1d1d1", 16))
    embed.set_author(name="MoBot Custom Commands", icon_url= client.user.avatar)
...
async def editCommandSession(args, message, client): # for editing commands
  embed = discord.Embed(color=int("0xd1d1d1", 16))
  embed.set_author(name="MoBot Custom Commands", icon_url=client.user.avatar, url="https://google.com/SimpleCommands/command_id=None/guild_id=%s/command_owner_id=%s/response_id=None/ref_msg_channel_id=None" % (message.guild.id, message.author.id))
